...
data class Action(
    /**
     * A string that identifies the action to be performed. Action names MUST be unique within the
     * set of actions for an entity. The behaviour of clients when parsing a Siren document that
     * violates this constraint is undefined. Required.
     *
    ...
    /**
     * The encoding type for the request. When omitted and the fields attribute exists, the default
  ...
  fun toBuilder(): Builder =
      newBuilder(name, href).clazz(clazz).method(method).title(title).type(type).fields(fields)
  ...
  class Builder internal constructor(private var name: String, private var href: URI) {
     ...
     *   within the set of actions for an entity. The behaviour of clients when parsing a Siren
     ...
     * Set value for class.
     *
     * @param clazz Describes the nature of an action based on the current representation. Possible
     ...
     *
     * @param method An enumerated attribute mapping to a protocol method. For HTTP, these values
     *   may be GET, PUT, POST, DELETE, or PATCH. As new methods are introduced, this list can be
     *   extended. If this attribute is omitted, GET should be assumed.
     * @return builder
     */
     ...
     * Set value for type.
     *
     * @param type The encoding type for the request. When omitted and the fields attribute exists,
  ...
  /**
   * An enumerated attribute mapping to a protocol method. For HTTP, these values may be GET, PUT,
   * POST, DELETE, or PATCH. As new methods are introduced, this list can be extended.
   */
  enum class Method {
    HEAD,
    GET,
  ...
  companion object {
    ...
    private fun fromRaw(map: Map<String, Any?>): Action =
        Action(
     ...
     */
    @JvmStatic fun newBuilder(name: String, href: URI): Builder = Builder(name, href)
  }
}
