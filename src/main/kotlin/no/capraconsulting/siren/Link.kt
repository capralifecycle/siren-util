...
/**
 * Links represent navigational transitions in the Siren specification. In JSON Siren, links are
 * represented as an array inside the entity, such as `{ "links": [{ "rel": [ "self" ], "href":
...
data class Link(
     ...
     *
     * An empty list will be excluded during serialization to JSON as this element is optional.
     *
  ...
  fun toBuilder(): Builder = newBuilder(rel, href).title(title).clazz(clazz).type(type)
  ...
  class Builder internal constructor(private var rel: List<String>, private var href: URI) {
     ...
     * Add value for class.
     *
     * @param clazz Describes aspects of the link based on the current representation. Possible
     ...
     * Add value for type.
     *
     * @param type Defines media type of the linked resource, per Web Linking (RFC5988). For the
     *   syntax, see RFC2045 (section 5.1), RFC4288 (section 4.2), RFC6838 (section 4.2)
     * @return builder
     */
    ...
    fun build() = Link(clazz = clazz, title = title, rel = rel, href = href, type = type)
  }
  ...
  companion object {
    ...
    private fun fromRaw(map: Map<String, Any?>): Link =
        Link(
     ...
     */
    @JvmStatic fun newBuilder(rel: List<String>, href: URI): Builder = Builder(rel, href)
     ...
     * @return a new builder
     */
    @JvmStatic fun newBuilder(rel: String, href: URI): Builder = Builder(listOf(rel), href)
  }
}
