...
data class Field(
    /**
     * A name describing the control. Field names MUST be unique within the set of fields for an
     * action. The behaviour of clients when parsing a Siren document that violates this constraint
  ...
  fun toBuilder() = newBuilder(name).clazz(clazz).type(type).title(title).value(value)
  ...
  class Builder internal constructor(private var name: String) {
     ...
     * Set value for name.
     *
     * @param name A name describing the control. Field names MUST be unique within the set of
     *   fields for an action. Required.
     * @return builder
     */
     ...
     * Set value for class.
     *
     * @param clazz Describes aspects of the field based on the current representation. Possible
     *   values areimplementation-dependent and should be documented.
     * @return builder
     */
     ...
     * Set value for type.
     *
     * @param type The input type of the field. This is a subset of the input types specified by
     *   HTML5.
     * @return builder
     */
    ...
    fun build() = Field(name = name, clazz = clazz, type = type, title = title, value = value)
  }
  ...
  companion object {
    ...
    private fun fromRaw(map: Map<String, Any?>): Field =
        Field(
     ...
     */
    @JvmStatic fun newBuilder(name: String): Builder = Builder(name)
  }
}
