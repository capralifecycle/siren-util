...
/**
 * An Entity is a URI-addressable resource that has properties and actions associated with it. It
 * may contain sub-entities and navigational links.
 *
 ...
 * @see Embedded
 *
 * Prefer using [newBuilder] to create a new instance instead of using the constructor, as the
 * constructor has some relaxed checking which should only be used when deserializing an existing
 * representation.
 */
data class Root(
    /**
   ...
   *
   * Only use this if you have full control over the Siren document as there is no guarantee what
   * will come first when having multiple class values.
   */
  val firstClass: String?
    get() = clazz.firstOrNull()
   ...
   *
   * The representation will be idempotent unless some special user data is used as properties that
   ...
   *
   * Attributes in the Siren specific structure that are null is not included as it covers optional
   * data. Attributes that is an empty list of empty map is treated as null and not included.
   *
   * @return object
   */
  ...
  class Builder internal constructor() {
     ...
     * Set value for links.
     *
     * @param links A collection of items that describe navigational links, distinct from entity
     *   relationships. Entities should include a link `rel` to `self`.
     * @return builder
     */
     ...
     * @return builder
     */
    fun entities(entities: List<Embedded>?) = apply { this.entities = entities ?: emptyList() }
     ...
     * @return builder
     */
    fun actions(actions: List<Action>?) = apply { this.actions = actions ?: emptyList() }
  ...
  companion object {
     ...
     *
     * If extra attributes not specified in the Siren specification is included they will be
     * discarded.
     *
     ...
     */
    @JvmStatic
    fun fromRaw(map: Map<String, Any?>): Root =
        Root(
     ...
     *
     * If extra attributes not specified in the Siren specification is present in the JSON value it
     * will be discarded.
     *
     * @param json valid Siren JSON value
     * @return a new Root
     */
    @JvmStatic fun fromJson(json: String): Root = fromRaw(json.parseJsonToMap().asMap())
